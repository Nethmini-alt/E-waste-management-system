...
import { X, Sparkles, ShoppingCart, Ship, AlertCircle } from 'lucide-react';
...
const GeneratePlanModal: React.FC<Props> = ({ open, onClose, onSubmit }) => {
  // Buyers
  ...
  const [route, setRoute] = useState<'' | 'LocalSale' | 'Export'>('');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setBuyers(all.filter((b) => b.status === 'Active'));
      } catch {
        setBuyers([]);
      } finally {
    ...
    (async () => {
      ...
      try {
        ...
        const salesOpts: OrderOption[] = sales.map((o) => ({
          ...
          status: o.status,
          materialTypes: Array.from(new Set(o.items.map((i) => i.materialType))),
          totalKg: o.items.reduce((s, i) => s + i.quantityKg, 0),
          route: 'LocalSale',
        }));
        ...
        // Newest first — group sales first then export
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const goal: any = {};
      if (buyerId) goal.targetBuyerId = buyerId;

      const mats = materialsText
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
      if (mats.length > 0) goal.targetMaterialTypes = mats;

      const kg = parseFloat(maxKg);
      if (!isNaN(kg) && kg > 0) goal.maxQuantityKg = kg;

      if (route) goal.preferredRoute = route;

      await onSubmit(goal);
      onClose();
    } catch (err: any) {
      // Keep modal open, show error inside
      setError(
      ...
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Sparkles size={18} /> Generate AI Commercial Plan
          </h3>
          <button onClick={onClose} style={iconBtn}><X size={18} /></button>
        </div>

        <p style={{ color: '#666', fontSize: 13, marginTop: 8 }}>
          Select a buyer and (optionally) an existing order to seed the agent's goal.
          Override any field before running.
        </p>
        ...
        <form onSubmit={submit} style={{ marginTop: 12 }}>
          ...
          <Field label="Buyer">
            <select
              ...
              style={input}
            >
              <option value="">— Any eligible buyer —</option>
              {loadingBuyers && <option disabled>Loading…</option>}
              {!loadingBuyers && buyers.map((b) => (
          ...
          {/* Orders (only when a buyer is picked) */}
          {buyerId && (
            <Field label="Seed from Existing Order (optional)">
              {loadingOrders ? (
                ...
                <select
                  ...
                  style={input}
                >
                  <option value="">— Start fresh (no prefill) —</option>
                  {orders.map((o) => (
                    <option key={o.id} value={o.id}>
                      {o.kind === 'export' ? '🌍' : '📦'} {o.label} · {o.totalKg.toFixed(1)}kg
                    </option>
                  ))}
                </select>
              )}
          ...
          {/* Preview of what the order will seed */}
          {selectedOrder && (
            <div style={prefillBox}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                {selectedOrder.kind === 'export'
                  ? <Ship size={14} color="#6a1b9a" />
                  : <ShoppingCart size={14} color="#1565c0" />}
                <strong style={{ fontSize: 13 }}>
                  Prefilled from {selectedOrder.kind === 'export' ? 'Export' : 'Local'} order
                </strong>
              </div>
              <div style={{ fontSize: 12, color: '#555' }}>
                <div>Materials: <strong>{selectedOrder.materialTypes.join(', ')}</strong></div>
                <div>Total quantity: <strong>{selectedOrder.totalKg.toFixed(2)} kg</strong></div>
                <div>Route preference: <strong>{selectedOrder.route === 'Export' ? 'Export' : 'Local Sale'}</strong></div>
              </div>
            </div>
          )}
          ...
          <Field label="Preferred Route">
            <select value={route} onChange={(e) => setRoute(e.target.value as any)} style={input}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 20 }}>
            ...
            <button type="submit" disabled={submitting} style={btnPrimary}>
              <Sparkles size={14} /> {submitting ? 'Running agent…' : 'Run Agent'}
            </button>
          </div>
        </form>
      </div>
...
const modal: React.CSSProperties = {
  background: '#fff', padding: 25, borderRadius: 10, width: 520,
...
const errorBox: React.CSSProperties = {
  marginTop: 12, padding: 10, background: '#ffebee', color: '#c62828',
