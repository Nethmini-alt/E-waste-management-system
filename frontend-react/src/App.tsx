...
import { 
  ...
  Tag, Weight, DollarSign, LayoutDashboard, Send, XCircle, Check, MapPin
} from 'lucide-react';
...
interface SubmissionResponse {
  ...
  userId?: string;
  userType?: string;
  pickupAddress?: string; // Added pickupAddress to interface
  status: string;
  createdAt?: string;
...
const App: React.FC = () => {
  ...
  const [pickupAddress, setPickupAddress] = useState<string>(''); // Added pickupAddress state
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      userType: "Generator",
      pickupAddress: pickupAddress, // Added pickupAddress to payload
      items: [
        {
  ...
  const handleAdminDecision = async (id: string, status: 'Approved' | 'Rejected') => {
    try {
      ...
      fetchAdminSubmissions(); // Refresh list after update
    } catch (err) {
      ...
      {activeTab === 'user' && (
        <div>
          ...
          <form onSubmit={handleSubmit} style={{ background: '#f9f9f9', padding: '20px', borderRadius: '8px', border: '1px solid #ddd' }}>
            ...
            <div style={{ marginBottom: '15px' }}>
              <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '5px' }}>Pickup Address:</label>
              <textarea 
                rows={2} 
                style={{ width: '100%', padding: '10px', borderRadius: '4px', border: '1px solid #ccc' }} 
          ...
          {submission && (
            <div style={{ marginTop: '20px', padding: '20px', background: '#ffffff', border: '1px solid #ddd', borderRadius: '8px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
              ...
              <p><strong>Pickup Address:</strong> {submission.pickupAddress || pickupAddress}</p>
              ...
              {allSubmissions.map((sub) => {
                ...
                return (
                  <div key={sub.id} style={{ background: '#fff', border: '1px solid #ddd', borderRadius: '8px', padding: '15px', display: 'flex', gap: '15px', alignItems: 'center' }}>
                    {item?.imageUrl && (
                      <img src={item.imageUrl} alt="E-Waste" style={{ width: '100px', height: '100px', objectFit: 'cover', borderRadius: '6px' }} />
                    )}
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <h4 style={{ margin: '0 0 5px 0' }}>{item?.itemName || 'E-Waste Item'}</h4>
                        <span style={{ fontSize: '12px', background: '#eee', padding: '3px 8px', borderRadius: '4px' }}>ID: {sub.id.substring(0, 8)}...</span>
                      </div>
                      <p style={{ margin: '0 0 4px 0', color: '#555', fontSize: '14px' }}>"{item?.description}"</p>
                      
                      {/* Display Pickup Address in Admin View */}
                      <p style={{ margin: '0 0 8px 0', color: '#2e7d32', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <MapPin size={14} /> <strong>Pickup Address:</strong> {sub.pickupAddress || 'Not Provided'}
                      </p>
                      
                      {ai ? (
                        <div style={{ fontSize: '13px', background: '#f5f5f5', padding: '8px', borderRadius: '4px', display: 'flex', gap: '15px' }}>
                          <span><strong>Category:</strong> {ai.wasteCategory}</span>
                          <span><strong>Hazard:</strong> <span style={{ color: ai.hazardLevel === 'High' || ai.hazardLevel === 'Critical' ? 'red' : 'green', fontWeight: 'bold' }}>{ai.hazardLevel}</span></span>
                          <span><strong>Value:</strong> ${ai.estimatedValueUsd}</span>
                        </div>
                      ) : (
