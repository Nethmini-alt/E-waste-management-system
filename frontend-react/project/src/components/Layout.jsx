...
import {
  ...
  IdCard, Boxes, PackagePlus, Wallet, Cpu, FileText, History, 
  BriefcaseBusiness, LayoutDashboard, Users, Tag, 
  ShoppingCart, Ship, DollarSign
} from 'lucide-react';
...
export const Layout = () => {
  ...
  // Anyone logged in who isn't staff/admin (and, by the time we get here,
  // isn't a collector either — see the early return below) is treated as
  // a customer for the purposes of the submission nav (household/corporate).
    ...
    <div className="relative flex min-h-screen overflow-hidden">
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto pr-1">
          ...
          <NavLink to="/submissions" className={({ isActive }) => `${navItem} ${isActive ? navItemActive : navItemInactive}`}>
            <BriefcaseBusiness size={16} /> Submission Hub
          </NavLink>

          {/* Admin / staff — operational tools for managing all submissions */}
          {isStaff && (
            <>
              <NavLink to="/submissions/history" className={({ isActive }) => `${navItem} ${isActive ? navItemActive : navItemInactive}`}>
                <History size={16} /> Submission History
              </NavLink>
              <NavLink to="/submissions/report/new" className={({ isActive }) => `${navItem} ${isActive ? navItemActive : navItemInactive}`}>
                <FileText size={16} /> Submission Report
              </NavLink>
            </>
          )}

          {/* Customer — their own intake + assessment flow */}
          {isCustomer && (
            <>
              <NavLink to="/submissions/new" className={({ isActive }) => `${navItem} ${isActive ? navItemActive : navItemInactive}`}>
                <Send size={16} /> Submit Item
              </NavLink>
              <NavLink to="/submissions/individual" className={({ isActive }) => `${navItem} ${isActive ? navItemActive : navItemInactive}`}>
                <Send size={16} /> Individual Submission
              </NavLink>
              <NavLink to="/submissions/cooperative" className={({ isActive }) => `${navItem} ${isActive ? navItemActive : navItemInactive}`}>
                <Users size={16} /> Cooperative Submission
              </NavLink>
            </>
          )}
