...
public class WorkflowService : IWorkflowService
{
    ...
    public async Task RecordAnalyzerResultAsync(Guid workflowId, AnalyzerResultRequest request, CancellationToken ct = default)
    {
        ...
        var submission = await _db.Submissions
            ...
            .FirstOrDefaultAsync(s => s.Id == workflow.SubmissionId, ct);

        if (submission is not null)
        {
            submission.Category = request.WasteCategory;
            submission.Status = "Pending_AI_Analysis";

            if (submission.AIAnalysis is null)
            {
                submission.AIAnalysis = new AIAnalysisResult
                {
                    SubmissionId = submission.Id,
                };
                _db.AIAnalysisResults.Add(submission.AIAnalysis);
            }

            submission.AIAnalysis.WasteCategory = request.WasteCategory;
            submission.AIAnalysis.EstimatedVolumeKg = request.EstimatedVolumeKg;
            submission.AIAnalysis.EstimatedValueUsd = request.EstimatedValueUsd;
            submission.AIAnalysis.HazardLevel = request.HazardLevel;
            submission.AIAnalysis.RequiresHumanApproval = false;
            submission.AIAnalysis.AnalyzedAt = DateTime.UtcNow;
        }
    ...
    public async Task RecordValidatorResultAsync(Guid workflowId, ValidatorResultRequest request, CancellationToken ct = default)
    {
        ...
        if (submission is not null && submission.AIAnalysis is not null)
        {
            submission.AIAnalysis.RequiresHumanApproval = request.RequiresHumanApproval;
            submission.Status = request.RequiresHumanApproval ? "Pending_Approval" : "Approved";
        }
